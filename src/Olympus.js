...
class Olympus {
    ...
    adjustItems(container) {
        const db = container.resolve("DatabaseServer").getTables();
        const items = db.templates.items;
        const { Resize, Sicc, KeyTool, MoneyCase } = require("./config.json");
        if (Resize) {
            if (Sicc.newHSize > 5) {
                items["5d235bb686f77443f4331278"]._props.Grids[0]._props.cellsH = Sicc.newHSize;
            }
            if (Sicc.newVSize > 5) {
                items["5d235bb686f77443f4331278"]._props.Grids[0]._props.cellsV = Sicc.newVSize;
            }
            if (KeyTool.newHSize > 4) {
                items["59fafd4b86f7745ca07e1232"]._props.Grids[0]._props.cellsH = KeyTool.newHSize;
            }
            if (KeyTool.newVSize > 4) {
                items["59fafd4b86f7745ca07e1232"]._props.Grids[0]._props.cellsV = KeyTool.newVSize;
            }
            if (MoneyCase.newHSize > 7) {
                items["59fb016586f7746d0d4b423a"]._props.Grids[0]._props.cellsH = MoneyCase.newHSize;
            }
            if (MoneyCase.newVSize > 7) {
                items["59fb016586f7746d0d4b423a"]._props.Grids[0]._props.cellsV = MoneyCase.newVSize;
            }
        }
    }
    pushItems(container) {
